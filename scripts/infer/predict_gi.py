...
def load_val_samples(val_file_path: str):
    ...
    提取 human(user) 的输入以及 gpt(assistant) 的真实标注 (GT)
    """
    samples = []
...
def main():
    ...
    args = {
        ...
        "adapter_name_or_path": "/media/inno/work_dirs/LLM/LlamaFactory/gi/outputs-sft-qwen3.5-4b-v1/checkpoint-318/",  # LLaMA-Factory 微调输出路径
        "template": "qwen3_5",
    ...
    # 3. 读取验证集数据
    val_file_path = '/media/inno/LLM/GI/TrainData/V1/sharegpt/val.jsonl'
    eval_samples = load_val_samples(val_file_path)
    print(f"总计成功解析待测样例数: {len(eval_samples)}")

    # 4. 指定输出路径
    output_dir = '/media/inno/output/LLM/gi/qwen3.5-4b-v1-llamafactory-gi/'
    os.makedirs(output_dir, exist_ok=True)
    val_json_path = os.path.join(output_dir, 'val.json')
    val_md_path = os.path.join(output_dir, 'val.md')
    ...
    # 5. 循环推理并对比评测结果
    print("\n开始 GI 多任务推理与 GT 结果对比评估...")
    start_inference_time = time.time()

    for idx, (user_input, gt_text) in enumerate(eval_samples, 1):
        sample_key = f"sample_{idx}"
        messages = [{"role": "user", "content": user_input}]

        sample_start_time = time.time()

        responses = chat_model.chat(messages, do_sample=False, max_new_tokens=2048)
        pred_text = responses[0].response_text
        response_len = responses[0].response_length
        total_generated_tokens += response_len

        # 保存原始 raw 输出到 val.json
        ...
        print(f"[耗时统计] 样本 {sample_key} 推理耗时: {sample_duration:.2f} 秒 (生成 {response_len} tokens)")
        print('=' * 60)
    ...
    print("-" * 50)
    print(f"5. 模型加载耗时: {init_duration:.2f} 秒")
    if torch.cuda.is_available():
        allocated_vram = torch.cuda.memory_allocated() / 1024 ** 2  # MB
        max_allocated_vram = torch.cuda.max_memory_allocated() / 1024 ** 2  # MB
    ...
    # 9. 保存 1: val.json（格式："{case_no}": raw）
    with open(val_json_path, 'w', encoding='utf-8') as f:
        json.dump(val_raw_results, f, ensure_ascii=False, indent=4)
    print(f"模型原始预测结果已成功保存至 JSON: {val_json_path}")

    # 10. 保存 2: val.md (对比评测报告与指标)
    md_lines = []
    md_lines.append("# GI 内镜报告生成模型评估对比报告 (val.md)\n")
    ...
    with open(val_md_path, 'w', encoding='utf-8') as f:
        f.write("\n".join(md_lines))
    print(f"模型评估对比报告已成功保存至 Markdown: {val_md_path}")
